...
* {@link #computeDerivedKey(SecretKey, int, Strin}.
*
...
namespace esapi
{
  class KeyDerivationFunction
  {
    ...
    * derived key from the {@code keyDerivationK} for either
    ...
    * It is used by ESAPI's reference crypto implementation class {@code JavaEncrypt}r
    ...
    * {@link #generateSecretKey(String, in}. The
    ...
    * @param keySizeThe cipher's key size (in bits) for the {@code keyDerivationK}.
    ...
    * {@code JavaEncrypt}, this must be either the string "encryption" or
    ...
    * be set to anything other than {@code nu} or an empty string when called outside
    * of {@code JavaEncrypt}.
    * @returnThe derived {@code SecretK} to be used according
    * to the specified purpose.
    * @throws NoSuchAlgorithmExceptionThe {@code keyDerivationK} has an unsupported
    ...
    * this is thrown with the original {@code UnsupportedEncodingExcepti}n
    ...
    * @returnTrue if {@code prfAlgNa} is supported, otherwise false.
    */
